...
function displayData(data) {
  ...
  const dataToDisplay = [
    ...
    `<i class="fas fa-thermometer"></i>${parseFloat(data[9])}mS/cm`]; // table of html data to displaying
  const dataToFind = ['thermometer', 'hygrometer', 'barometer', 'light', 'balance-in', 'balance-out', 'thermometer-out', 'ec-out', 'battery-level', 'date', 'time'];
  // Weather chart (Temperature, Humidity, Pressure, Light)
  for(let i=0; i<=3;i++) {
    // data length 3h
    if(chartWeather.series[i].data.length > 8640) {
      chartWeather.series[i].addPoint([today, parseFloat(data[i+2])], true, true, true);
    } else {
      chartWeather.series[i].addPoint([today, parseFloat(data[i+2])], true, false, true);
    }
    const valueToDisplay = document.getElementById(dataToFind[i]);
    valueToDisplay.innerHTML = dataToDisplay[i];  // adding to html the value (upon the charts)
  }
  // Waterflow chart (Water In and Out)
  for(let i=0; i<=1;i++) {
    if(chartWaterFlow.series[i].data.length > 8640) {
      chartWaterFlow.series[i].addPoint([today, parseFloat(data[i+6])], true, true, true);
    } else {
      chartWaterFlow.series[i].addPoint([today, parseFloat(data[i+6])], true, false, true);
    }
  ...
  for(let i=0; i<=1;i++) {
    if(chartWaterParams.series[i].data.length > 8640) {
      chartWaterParams.series[i].addPoint([today, parseFloat(data[i+8])], true, true, true);
    } else {
      chartWaterParams.series[i].addPoint([today, parseFloat(data[i+8])], true, false, true);
    }
  ...
  valueToDisplay.innerHTML = d.getDate() + "." + d.getMonth() + "." + d.getFullYear(); // adding date and time to html
  ...
  valueToDisplay.innerHTML = parseFloat(data[10]) + '%';  // adding battery level value to html
  if(parseInt(data[10]) <= 15) {
    valueToDisplay.style.color = "red";   // if battery level is lower than 15% than color the battery red
  } else {
...
function saveDatabase(data) {
  ...
  ? JSON.parse(localStorage.getItem('Database'))  // check if there is in local storage item database if not then create it
  : []

  var databaseArray = data.split('\n'); // split data in rows
...
//Automaticly after sacing databse send command to delete buffer.csv from the SD Card
...
function createDatabaseTable(){
  ...
  const databaseArray = JSON.parse(localStorage.getItem('Database')); // save in constant the database saved in local storage
...
for(let link of this.navLinks) {
  link.addEventListener('click', function(event) {
    ...
    let href = link.getAttribute('href'); // read the hred of each page
    let id = href.substring(1);   // read the id from href of each page (delete the '#')
    activePage(id); // dislay the selected page
    // If the nav clicked is database, display loader while the database table is created
    if(id === 'database') {
      const loader = document.querySelector(".loader");
      loader.classList.add("show");
      // Timeout needed to display the loader properly. Creating the database table (loader turn off after table is loaded)
      setTimeout(function() {
        createDatabaseTable()
      }, 250);
    }
...
function refreshPage(){
  const loader = document.querySelector(".loader");
  loader.classList.add("show");
  //updateData();   // if off, than wait 5 seconds, if on than updating automaticly after refreshing the page
  activePage('live'); // First page (Charts) after refresh
  checkId();  // upload the device id
  //updateDatabase(); // if off, than afte 60seconds the database is uploded, if on then automaticly (can cause stucking!)
  loader.classList.remove("show");
}
  ...
  xhttp.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
      ...
      deviceIds.forEach(deviceId => {
        deviceId.innerHTML = deviceName;    // display the device ID in html (up top of the page)
      })
    }
  }
...
function calibrationPopup(choosenSetting) {
  ...
  const analogRead = document.querySelector(`.analog-${choosenSetting}`); // Temporarly displaying the analog reads from the selected device to calibrate
  ...
  // Start the calibration function (send info to the device, display info on screen)
...
// Actions for selected settings (delete database, download database, restart device)
...
function downloadDatabase() {
  ...
  xhttp.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
      window.open('/download_database', '_blank'); // '_blank' needed for not opening another page
